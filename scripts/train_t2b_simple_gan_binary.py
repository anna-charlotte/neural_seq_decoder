...
class Generator(nn.Module):
    def __init__(self, ngpu, nz, ngf, nc):
        ...
        self.main = nn.Sequential(
            ...
            nn.ConvTranspose2d(
                in_channels=nz, out_channels=ngf * 8, kernel_size=(8, 4), stride=(1, 1), padding=0, bias=False
            ),  # (8, 4)
            nn.BatchNorm2d(ngf * 8),
            nn.ReLU(True),
            nn.ConvTranspose2d(
            ...
            nn.ReLU(True),
            nn.ConvTranspose2d(
                in_channels=ngf, out_channels=nc, kernel_size=(6, 4), stride=(4, 2), padding=1, bias=False
            ),  # (256, 32)
            nn.Tanh(),
        )
        ...
        for sample_tensor in samples_:
            ...
            for avg_phoneme_cls, avg_tensor in sorted(averaged.items()):
                if distance_metric == "frobenius":
                    ...
                    raise ValueError(f"Distance metric is invalid: {distance_metric}. Valid options are: .")
...
def main(args: dict) -> None:
    ...
    eval_dir = (
        ...
        / "evaluation"
        / "gan_binary"
        / f"{timestamp}__lrd_{args['lr_d']}__lrg_{args['lr_g']}__with_64x64_DCGAN"
    )
        ...
        for i, data in enumerate(train_dl, 0):
            ...
            if i % 50 == 0:
                print(
                    ...
                    % (epoch, num_epochs, i, len(train_dl), errD.item(), errG.item(), D_x, D_G_z1, D_G_z2)
                )
                with torch.no_grad():
...
if __name__ == "__main__":
    for lr_d, lr_g in zip(
        [0.0002, 0.00001, 0.00001, 0.00001, 0.00002, 0.00002, 0.00005, 0.00005, 0.00005],
        [0.0002, 0.0001, 0.0002, 0.00005, 0.0001, 0.0002, 0.0001, 0.0002, 0.0005],
    ):
        now = datetime.now()
